  ...
  <div class="w-96 h-auto overflow-hidden md:p-5 p-4 bg-white rounded-2xl inline-block">
    <div class="flex items-center space-x-2 mb-6">
      ...
      @if ($item->user->detail_user->photo != null)
        <img class="w-14 h-14 object-cover rounded-full mr-1" src="{{ url(Storage::url($item->user->detail_user->photo)) }}" alt="photo freelancer"
          loading="lazy" />
      @else
    ...
    @if ($item->thumbnail_service[0]->thumbnail != null)
      <img class="rounded-2xl object-cover h-48 w-full" src="{{ url(Storage::url($item->thumbnail_service[0]->thumbnail)) }}" alt="photo freelancer"
        loading="lazy" />
    @else
